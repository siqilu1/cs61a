...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        ...
        # print(roll_dice(num_rolls,dice))
...
def swine_align(player_score, opponent_score):
    ...
    while not (opponent_score % GCD == 0 and player_score % GCD == 0): 
        GCD -= 1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say_highest(score0, score1):
        if who == 0:
            ...
            now_score = score1
        score_gained = now_score - last_score
        if score_gained > running_high:
            print(score_gained, 'point(s)! The most yet for Player', who)
            return announce_highest(who, now_score, score_gained)
        else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    k, highest, lowest_k = 1, 0, 1
    while k <= 10:
        trial, result = 1, 0
        while trial <= trials_count:
            trial, result = trial + 1, result + roll_dice(k, dice)
        if result > highest:
            highest, lowest_k = result, k
        k += 1
    return lowest_k
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_after_0_dice = score + free_bacon(opponent_score)
    if extra_turn(score_after_0_dice, opponent_score):
        return 0
    else:
